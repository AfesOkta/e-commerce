    ...
    return ( <
        ...
        div className = "relative inline-block w-10 mr-2 align-middle select-none" > {
            warna === "dark" ? ( <
                ...
                name = "toggle"
                id = "toggle"
                defaultChecked className = "absolute block w-6 h-6 transition border-4 rounded-full appearance-none cursor-pointer bg-secondary-900 border-secondary-700 toggle-checkbox focus:outline-none" /
                >
            ) : ( <
                input onClick = {
                    () => selectTheme("dark") }
        ...
        < /label> <
        /div> <
        label className = "text-xs select-none dark:text-gray-200" > { warna == "dark" ? "Dark mode!" : "Light mode!" } <
        /label> <
        /div>
    );
}
